...
name_to_ex = {e: re.compile(rf'\b{re.escape(e)}\b',
                            re.IGNORECASE) for e in ex_names}
...
all_ex = re.compile(r'\b(' + '|'.join(re.escape(e)
                    for e in ex_names) + r')\b', re.IGNORECASE)
...
def get_description(score):
    if score >= 0.5:
        return "People really enjoy this exercise and find it efficient!"
    elif score >= 0.2:
        return "This is an efficient exercise for working out."
    elif score >= 0.0:
        return "This is a pretty good exercise."
    elif score >= -0.2:
        return "This exercise might be more situational."
...
for _, row in exercises_df.iterrows():
    ...
    if comments:
        score = round(sum(sia.polarity_scores(
            c)['compound'] for c in comments) / len(comments), 3)
    else:
